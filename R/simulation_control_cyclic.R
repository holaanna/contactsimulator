#' Simulation of the epidemic control process with cyclic infectious period
#'
...
#'
#' @export
Simulate_contact_control2<- function(f_rast=NULL, b_rast=NULL, farm_pos_cat=NULL, vis_int_per_cat=NULL, param, grid_lines, pop_grid, grid_size=500, age_level=c(1,1),age_dist=c(1,0), m_start=1,t_b=100000, t_max=1000, t_intervention=100000, t_obs=3703, EI_model=1, kern_model=4,rad=1000,sweep_prop=c(.5,.5),back_p=c(.7,.5),rate_det=0.3,int_det=c(30,90,180),nb_in_b=1,nb=30,leav=c(3,6)){
  ...
  index_t_r<- numeric(m_start)

  for(i in 1:m_start){
    index_t_r[i] <- index_t_i[i] + r_IR(1,index_t_i[i],c,alpha2)
  ...
  while(t_next<(t_max)){
    ...
    if(t_now<t_obs){
      t_r_new <- t_i_new + r_IR(1,t_i_new ,c,alpha2)
      if(t_r_new>t_obs){
        t_r_new <- 2*t_max
      }
    }
