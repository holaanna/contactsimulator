#' Simulation of the epidemic control process old version
#'
  ...
  while(t_next<(t_max)){
      ...
      switch (kk,
              ...
              {  # control sweep
                ...
                for(i in 1:length(indx_con)){
                  ...
                  if(ll==1){  # baseline
                    baseline_con_v0(farm_pos_cat,farm_inf,farm_inf_on,min_tim_cont,indx_rem,indx_con[i],mm_1,nn_1,int_det)
                  }
                  else{ # alternative
                    baseline_alt_v0(farm_pos_cat,vis_int_per_cat,farm_inf,farm_inf_on,min_tim_cont,indx_rem,indx_con[i],mm_1,nn_1)
                  }
    ...
    while(x_new<min_coor_x | x_new>max_coor_x | y_new<min_coor_y | y_new>max_coor_y){
      ...
      if (source!=9999){
        ...
        #print(c(r,n_set_points))
        if (n_set_points<1) {
          if(b_rast[simulated_epi[source+1,"row"],simulated_epi[source+1,"col"]]==0 & f_rast[simulated_epi[source+1,"row"],simulated_epi[source+1,"col"]]==0){
            k_grid <- sample(1:length(as.numeric(pop_grid)),size=1, prob=as.numeric(pop_grid))
            m_grid <- k_grid%%nrow(pop_grid) # the mth row of the grids
            if(m_grid==0) m_grid <- nrow(pop_grid)
            n_grid <- ceiling(k_grid/nrow(pop_grid))  # nth column ..
